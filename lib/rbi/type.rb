...
module RBI
  ...
  class Type
    ...
    class Shape < Type
      #: T::Hash[(String | Symbol), Type]
      attr_reader :types

      #: (T::Hash[(String | Symbol), Type] types) -> void
      def initialize(types)
        super()
        @types = types
      end
    ...
    class << self
      ...
      #: (String name, *(Type | T::Array[Type]) params) -> Generic
      def generic(name, *params)
        unsafe = Generic #:: untyped
        unsafe.new(name, *params.flatten)
      end
